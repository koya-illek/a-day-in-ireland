...
const normalizeStationName = (value) => String(value ?? "").trim().toLocaleLowerCase("en-IE");

export function matchesWeatherStationIdentity(station, providerName) {
  return normalizeStationName(providerName) === normalizeStationName(station.providerName);
}

export function isWeatherObservationFresh(observedAt, now = Date.now()) {
